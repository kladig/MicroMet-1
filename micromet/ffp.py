...
class ffp_climatology:
    ...
    def run_ffp(
        ...
        xmin, xmax, ymin, ymax, nx, ny, dx, dy = self.define_computational_domain(
            dx, dy, nx, ny, domain
        )

        self.pulse = self.define_pulse(self.pulse)

        x_2d, y_2d, rho, theta, fclim_2d = self.define_physical_domain(
            xmin, xmax, ymin, ymax, nx, ny
        )

        fclim_2d, f_2d, valids = self.loop_on_time_series(
            ustars,
            sigmavs,
            hs,
            ols,
        ...
        xrs, yrs, frs = self.derive_footprint_ellipsoid(
            fclim_2d, dx, dy, x_2d, y_2d, self.rs
        )

        x_2d, y_2d, fclim_2d = self.crop_domain_and_footprint(
            xrs, yrs, x_2d, y_2d, xmin, xmax, ymin, ymax, self.rs, fclim_2d
        )
    ...
    def define_computational_domain(self, dx, dy, nx, ny, domain):
        ...
        if isinstance(dy, (float, int)) and dx is None:
            dx = dy
        if not all(
            isinstance(item, (float, int)) for item in [dx, dy] for item in [dx, dy]
        ):
            dx = dy = None
    ...
    def loop_on_time_series(
        ...
        for ix, params in enumerate(
            ...
            if not valids[ix]:
                ...
                rotated_theta = (
                    theta - wind_dir * pi_over_180 if wind_dir is not None else theta
                )
    ...
    def crop_domain_and_footprint(
        self, xrs, yrs, x_2d, y_2d, xmin, xmax, ymin, ymax, rs, fclim_2d
    ):
        # ===========================================================================
...
def ref_et_from_nldas(
    ...
    df = nldas.get_bycoords(
        [(longitude, latitude)],
        start_date=f"{date:%Y-%m-%d}",
        end_date=f"{date + pd.DateOffset(days=1):%Y-%m-%d}",
        source="netcdf",
    )
